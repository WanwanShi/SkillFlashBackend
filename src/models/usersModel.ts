...
import { patchDeckUsername } from "./decksModel";
...
const updateUserByUsername = async (
  ...
  if (userDecks) {
    const updateAllDecks = (decks: Deck[]) => {
      const patchAllDecks: Promise<boolean>[] = decks.map(async (deck) => {
        deck.username = updatedUser.username;
        return await patchDeckUsername(deck._id.toString(), deck);
      });

      return Promise.all(patchAllDecks);
    };
    const areAllDecksUpdated = await updateAllDecks(userDecks);

    if (!areAllDecksUpdated.every((element) => element === true)) {
      return Promise.reject({
        status: 400,
        message: "User decks failed to update",
      });
    }
  }
